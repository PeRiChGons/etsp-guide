...
(function () {
  ...
  function renderSpirit(page) {
    ...
    return spiritHero + pageHeader(
      ...
      '<h3>Arraycore y evolución</h3>' + tags(treasure.arraycore, false) + facts(treasure.arrayFacts) + '<aside class="verification-note"><strong>Campos sin dato:</strong> costes: ? · monedas: ? · recompensas completas: ? · relación con Wisp Voucher: ?</aside></section>' +
      ...
      '<h3>Datos confirmados</h3>' + facts(bond.confirmed) + '<aside class="verification-note"><strong>Campos sin dato:</strong> costes: ? · umbrales: ? · efectos: ? · obtención de afinidad: ?</aside></section>' +
  ...
  function renderActivities(page) {
    ...
    return pageHeader(page, 'Actividades del juego', 'Mazmorras, jefes y actividades documentadas a partir de las grabaciones disponibles.', 'Se publica únicamente lo que aparece en una pantalla identificable. Costes, reinicios y tablas completas quedan separados como pendientes.') +
      '<section class="system-section activity-overview" id="vista-general"><h2>Vista general</h2><p>Estas actividades se incorporan al plano porque aparecen en los vídeos de juego. Cada una tendrá su ficha de recompensas, requisitos y rutas.</p><div class="fact-grid"><article class="fact-card"><h3>Actividades observadas</h3>' + tags(data.observed, false) + '</article><article class="fact-card"><h3>Campos sin dato</h3>' + tags(data.pending, true) + '</article></div></section>' +
